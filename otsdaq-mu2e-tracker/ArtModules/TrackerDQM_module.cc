...
int TrackerDQM::dtcIndex(int DtcID)
{
	...
	if(DtcID == 0)
		...
		dtc_index = 1;  // daq09, new
	else if(DtcID == 19)
		dtc_index = 0;  // daq09, new
	else if(DtcID == 42)
		dtc_index = 1;  // daq09, old
	else if(DtcID == 73)
		dtc_index = 0;  // daq09, old
	else if(DtcID == 44)
		dtc_index = 0;  // daq22
	else if(DtcID == 45)
		dtc_index = 1;  // daq22
...
//-----------------------------------------------------------------------------
void TrackerDQM::unpack_adc_waveform(mu2e::TrackerDataDecoder::TrackerDataPacket* Hit,
                                     float*                                       Wf,
                                     WfParam_t*                                   Wp)
{
	...
	{
		mu2e::TrackerDataDecoder::TrackerADCPacket* ahit =
		    (mu2e::TrackerDataDecoder::TrackerADCPacket*)(((uint16_t*)Hit) + 8 + 8 * i);
		int loc = 12 * i + 2;
    ...
    _port(conf().port())
{
	...
	if(_pulserFrequency == 60)
		_freq = f0 / (pow(2, 9) + 1);  // ~ 60 kHz
	else if(_pulserFrequency == 250)
		_freq = f0 / (pow(2, 7) + 1);  // ~250 kHz

	_dt = 1 / _freq * 1.e9;  // in ns
	//------------------------------------------------------------------------------
	// default map, Richie says TS1 may have an old firmware with some bugs
	//-----------------------------------------------------------------------------
	int adc_index_0[96] = {91, 85, 79, 73, 67, 61, 55, 49, 43, 37, 31, 25,
	                       19, 13, 7,  1,  90, 84, 78, 72, 66, 60, 54, 48,

	                       42, 36, 30, 24, 18, 12, 6,  0,  93, 87, 81, 75,
	                       69, 63, 57, 51, 45, 39, 33, 27, 21, 15, 9,  3,

	                       44, 38, 32, 26, 20, 14, 8,  2,  92, 86, 80, 74,
	                       68, 62, 56, 50, 47, 41, 35, 29, 23, 17, 11, 5,

	                       95, 89, 83, 77, 71, 65, 59, 53, 46, 40, 34, 28,
	                       22, 16, 10, 4,  94, 88, 82, 76, 70, 64, 58, 52};
	//-----------------------------------------------------------------------------
...
// I : channel number
//-----------------------------------------------------------------------------
void TrackerDQM::book_channel_histograms(
    art::TFileDirectory* Dir, int RunNumber, ChannelHist_t* Hist, int Link, int I)
{
	Hist->nhits =
	    Dir->make<TH1F>(Form("ch_%i_%02i_nhits", Link, I),
	                    Form("run %06i: link %i ch %02i nhits", RunNumber, Link, I),
	                    50,
	                    -0.5,
	                    49.5);
	Hist->time[0] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_time0", Link, I),
	                    Form("run %06i: link %i ch %02i time[0]", RunNumber, Link, I),
	                    1000,
	                    0.,
	                    100.);  // us
	Hist->time[1] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_time1", Link, I),
	                    Form("run %06i: link %i ch %02i time[0]", RunNumber, Link, I),
	                    1000,
	                    0.,
	                    100.);  // us
	Hist->t0[0] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_t0_0", Link, I),
	                    Form("run %06i: link %i ch %02i t0[0]  ", RunNumber, Link, I),
	                    1000,
	                    -20.,
	                    80.);  // ns
	Hist->t0[1] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_t0_1", Link, I),
	                    Form("run %06i: link %i ch %02i t0[1]  ", RunNumber, Link, I),
	                    1000,
	                    -20.,
	                    80.);  // ns
	Hist->t1[0] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_t1_0", Link, I),
	                    Form("run %06i: link %i ch %02i t1[0]  ", RunNumber, Link, I),
	                    1000,
	                    -20.,
	                    80.);  // ns
	Hist->t1[1] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_t1_1", Link, I),
	                    Form("run %06i: link %i ch %02i t1[1]  ", RunNumber, Link, I),
	                    1000,
	                    -20.,
	                    80.);  // ns
	Hist->tot[0] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_tot0", Link, I),
	                    Form("run %06i: link %i ch %02i tot[0]", RunNumber, Link, I),
	                    100,
	                    0.,
	                    100.);
	Hist->tot[1] =
	    Dir->make<TH1F>(Form("ch_%i_%02i_tot1", Link, I),
	                    Form("run %06i: link %i ch %02i tot[1]", RunNumber, Link, I),
	                    100,
	                    0.,
	                    100.);
	Hist->pmp     = Dir->make<TH1F>(Form("ch_%i_%02i_pmp", Link, I),
                                Form("run %06i: link %i ch %02i pmp", RunNumber, Link, I),
                                100,
                                0.,
                                10.);
	Hist->dt01[0] = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt01_0", Link, I),
	    Form("run %06i: link %i ch %02i T0(i)-T1(i) [0],ns", RunNumber, Link, I),
	    500,
	    -25,
	    25);
	Hist->dt01[1] = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt01_1", Link, I),
	    Form("run %06i: link %i ch %02i T0(i)-T1(i) [1],ns", RunNumber, Link, I),
	    500,
	    -2500,
	    2500);
	Hist->dt0 = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt0", Link, I),
	    Form("run %06i: link %i ch %02i T0(i+1)-T0(i)", RunNumber, Link, I),
	    50000,
	    0.,
	    50);
	Hist->dt1 = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt1", Link, I),
	    Form("run %06i: link %i ch %02i T1(i+1)-T1(i)", RunNumber, Link, I),
	    50000,
	    0.,
	    50);
	Hist->dt2 = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt2", Link, I),
	    Form("run %06i: link %i ch %02i T2(i+1)-T2(i)", RunNumber, Link, I),
	    50000,
	    0.,
	    50);
	Hist->dt0r = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt0r_0", Link, I),
	    Form("run %06i: link %i ch %02i T0(ich,0)-T0(ref,0)[0]", RunNumber, Link, I),
	    20000,
	    -10.,
	    10);
	Hist->dt1r = Dir->make<TH1F>(
	    Form("ch_%i_%02i_dt1r_0", Link, I),
	    Form("run %06i: link %i ch %02i T1(ich,0)-T1(ref,0)[0]", RunNumber, Link, I),
	    20000,
	    -10.,
	    10);
	//-----------------------------------------------------------------------------
	...
	{
		Hist->fsample = Dir->make<TH1F>(
		    Form("ch_%i_%02i_fs", Link, I),
		    Form("run %06i: link %i ch %02i first sample", RunNumber, Link, I),
		    30,
		    -0.5,
		    29.5);
		Hist->bline = Dir->make<TH1F>(
		    Form("ch_%i_%02i_bl", Link, I),
		    Form("run %06i: link %i ch %02i WF baseline", RunNumber, Link, I),
		    250,
		    0,
		    500);
		Hist->pheight = Dir->make<TH1F>(
		    Form("ch_%i_%02i_ph", Link, I),
		    Form("run %06i: link %i ch %02i WF pulse height", RunNumber, Link, I),
		    500,
		    0,
		    500);
		Hist->q = Dir->make<TH1F>(
		    Form("ch_%i_%02i_q", Link, I),
		    Form("run %06i: link %i ch %02i WF charge", RunNumber, Link, I),
		    500,
		    0,
		    500);  //
		Hist->qt = Dir->make<TH1F>(
		    Form("ch_%i_%02i_qt", Link, I),
		    Form("run %06i: link %i ch %02i WF tail charge", RunNumber, Link, I),
		    500,
		    0,
		    500);
		Hist->qtq =
		    Dir->make<TH1F>(Form("ch_%i_%02i_qtq", Link, I),
		                    Form("run %06i: link %i ch %02i WF Qt/Q", RunNumber, Link, I),
		                    200,
		                    0,
		                    1);
		//-----------------------------------------------------------------------------
		...
		{
			Hist->raw_wf[j] =
			    Dir->make<TH1F>(Form("raw_wf_ch_%i_%02i_%i", Link, I, j),
			                    Form("run %06i: link %i ch [%02i][%i] raw_waveform",
			                         RunNumber,
			                         Link,
			                         I,
			                         j),
			                    ...
			                    0.,
			                    30.);
			Hist->wf[j] = Dir->make<TH1F>(
			    Form("wf_ch_%i_%02i_%i", Link, I, j),
			    Form("run %06i: link %i ch [%02i][%i] waveform", RunNumber, Link, I, j),
			    30,
			    0.,
			    30.);
		}
...
//-----------------------------------------------------------------------------
void TrackerDQM::book_dtc_histograms(
    art::TFileDirectory* Dir, int RunNumber, DtcHist_t* Hist, int IStation, int IDtc)
{
}

//-----------------------------------------------------------------------------
void TrackerDQM::book_roc_histograms(art::TFileDirectory* Dir,
                                     int                  RunNumber,
                                     RocHist_t*           Hist,
                                     int                  Station,
                                     int                  Dtc,
                                     int                  Link)
{
	Hist->nbytes = Dir->make<TH1F>(
	    "nbytes",
	    Form("run %06i: link_%02i:%i:%i n bytes", RunNumber, Station, Dtc, Link),
	    10000,
	    0.,
	    10000.);
	Hist->npackets = Dir->make<TH1F>(
	    "npackets",
	    Form("run %06i: link %02i:%i:%i n packets", RunNumber, Station, Dtc, Link),
	    1000,
	    0.,
	    1000.);
	Hist->nhits = Dir->make<TH1F>(
	    "nhits",
	    Form("run %06i: link %02i:%i:%i n hits", RunNumber, Station, Dtc, Link),
	    300,
	    0.,
	    300.);
	Hist->valid = Dir->make<TH1F>(
	    "valid",
	    Form("run %06i: link %02i:%i:%i valid", RunNumber, Station, Dtc, Link),
	    2,
	    0.,
	    2.);
	Hist->error_code = Dir->make<TH1F>(
	    "errcode",
	    Form("run %06i: link %02i:%i:%i errcode", RunNumber, Station, Dtc, Link),
	    512,
	    0.,
	    512.);

	Hist->n_empty =
	    Dir->make<TH1F>("nempt", Form("run %06i: N(empty)", RunNumber), 100, 0., 100.);
	Hist->n_invalid_dr = Dir->make<TH1F>(
	    "ninvr", Form("run %06i: N(invalid DR)", RunNumber), 100, 0., 100.);
	Hist->n_corrupt =
	    Dir->make<TH1F>("ncorr", Form("run %06i: N(corrupt)", RunNumber), 100, 0., 100.);
	Hist->n_timeouts =
	    Dir->make<TH1F>("ntmo", Form("run %06i: N(timeouts)", RunNumber), 100, 0., 100.);
	Hist->n_overflows = Dir->make<TH1F>(
	    "nover", Form("run %06i: N(overflows)", RunNumber), 100, 0., 100.);

	Hist->nerr_tot = Dir->make<TH1F>(
	    "nerr_tot",
	    Form("run %06i: link %02i:%i:%i N errors", RunNumber, Station, Dtc, Link),
	    1000,
	    0.,
	    2000.);
	Hist->nerr_vs_evt = Dir->make<TH1F>(
	    "nerr_vs_evt",
	    Form("run %06i: link %02i:%i:%i N err vs evt", RunNumber, Station, Dtc, Link),
	    1000,
	    0.,
	    5000000.);
	Hist->eflg_vs_evt = Dir->make<TH1F>(
	    "eflg_vs_evt",
	    Form("run %06i: link %02i:%i:%i eflag vs evt", RunNumber, Station, Dtc, Link),
	    1000,
	    0.,
	    5000000.);

	Hist->nh_vs_ch = Dir->make<TH2F>(
	    "nh_vs_ch",
	    Form("run %06i: link %02i:%i:%i nh vs ch", RunNumber, Station, Dtc, Link),
	    100,
	    0.,
	    100.,
	    50,
	    0,
	    50);
	Hist->nh_vs_adc0 = Dir->make<TH2F>(
	    "nh_vs_adc0",
	    Form("run %06i: link %02i:%i:%i nh vs adc", RunNumber, Station, Dtc, Link),
	    100,
	    0.,
	    100.,
	    50,
	...
	Hist->dt0r_vs_ch = Dir->make<TH2F>(
	    "dt0r_vs_ch_0",
	    Form("run %06i: link %02i:%i:%i dt0r vs ch[0]", RunNumber, Station, Dtc, Link),
	    100,
	    0.,
	    100.,
	    2500,
	    -25,
	    25);
	Hist->dt1r_vs_ch = Dir->make<TH2F>(
	    "dt1r_vs_ch_0",
	    Form("run %06i: link %02i:%i:%i dt1r vs ch[0]", RunNumber, Station, Dtc, Link),
	    100,
	    0.,
	    100.,
	    2500,
	...
	Hist->dt0r01 = Dir->make<TH1F>(
	    "dt0r01",
	    Form("run %06i: link %02i:%i:%i dt0r01", RunNumber, Station, Dtc, Link),
	    40000,
	    -20000,
	    20000);
	Hist->dt1r01 = Dir->make<TH1F>(
	    "dt1r01",
	    Form("run %06i: link %02i:%i:%i dt1r01", RunNumber, Station, Dtc, Link),
	    40000,
	    -20000,
	    20000);

	Hist->nhits_vs_ich = Dir->make<TH1F>(
	    "nh_vs_ich",
	    Form("run %06i: link %02i:%i:%i nh vs ich", RunNumber, Station, Dtc, Link),
	    100,
	    0.,
	    100.);
	Hist->nhits_vs_adc = Dir->make<TH1F>(
	    "nh_vs_adc",
	    Form("run %06i: link %02i:%i:%i nh vs adc", RunNumber, Station, Dtc, Link),
	    100,
	    0.,
	    100.);
	// Hist->nhits_vs_adc[1] = Dir->make<TH1F>("nh_vs_adc_1",     Form("run %06i: link
	// %02i:%i:%i nh vs adc_1"  ,RunNumber,Station,Dtc,Link),  100, 0.,   100.);

	Hist->dt0rc_vs_ch[0] =
	    Dir->make<TH2F>("dt0rc_vs_ch_0",
	                    Form("run %06i: link %02i:%i:%i dt0rc vs ch[0], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	                    ...
	                    10);
	Hist->dt0rc_vs_ch[1] =
	    Dir->make<TH2F>("dt0rc_vs_ch_1",
	                    Form("run %06i: link %02i:%i:%i dt0rc vs ch[1], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	...
	Hist->dt1rc_vs_ch[0] =
	    Dir->make<TH2F>("dt1rc_vs_ch_0",
	                    Form("run %06i: link %02i:%i:%i dt1rc vs ch[0], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	                    ...
	                    10);
	Hist->dt1rc_vs_ch[1] =
	    Dir->make<TH2F>("dt1rc_vs_ch_1",
	                    Form("run %06i: link %02i:%i:%i dt1rc vs ch[1], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	...
	Hist->dt0rc_vs_adc[0] =
	    Dir->make<TH2F>("dt0rc_vs_adc_0",
	                    Form("run %06i: link %02i:%i:%i dt0rc vs adc[0], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	                    ...
	                    10);
	Hist->dt0rc_vs_adc[1] =
	    Dir->make<TH2F>("dt0rc_vs_adc_1",
	                    Form("run %06i: link %02i:%i:%i dt0rc vs adc[1], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	...
	Hist->dt1rc_vs_adc[0] =
	    Dir->make<TH2F>("dt1rc_vs_adc_0",
	                    Form("run %06i: link %02i:%i:%i dt1rc vs adc[0], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	                    ...
	                    10);
	Hist->dt1rc_vs_adc[1] =
	    Dir->make<TH2F>("dt1rc_vs_adc_1",
	                    Form("run %06i: link %02i:%i:%i dt1rc vs adc[1], ns",
	                         RunNumber,
	                         Station,
	                         Dtc,
	                         Link),
	...
	if(_fillWfHistograms)
	{
		Hist->fs_vs_ich = Dir->make<TProfile>(
		    "fs_vs_ich",
		    Form("run %06i: link %02i:%i:%i fs vs ich", RunNumber, Station, Dtc, Link),
		    100,
		    0.,
		    100.,
		    0,
		    30);
		Hist->bl_vs_ich = Dir->make<TProfile>(
		    "bl_vs_ich",
		    Form("run %06i: link %02i:%i:%i bl vs ich", RunNumber, Station, Dtc, Link),
		    100,
		    0.,
		    100.,
		    0,
		    500);
		Hist->ph_vs_ich = Dir->make<TProfile>(
		    "ph_vs_ich",
		    Form("run %06i: link %02i:%i:%i ph vs ich", RunNumber, Station, Dtc, Link),
		    100,
		    0.,
		    100.,
		    0,
		    500);
		Hist->q_vs_ich = Dir->make<TProfile>(
		    "q_vs_ich",
		    Form("run %06i: link %02i:%i:%i Q vs ich", RunNumber, Station, Dtc, Link),
		    100,
		    0.,
		    100.,
		    0,
		    1500);
		Hist->qt_vs_ich = Dir->make<TProfile>(
		    "qt_vs_ich",
		    Form("run %06i: link %02i:%i:%i Qt vs ich", RunNumber, Station, Dtc, Link),
		    100,
		    0.,
		    100.,
		    0,
		    500);
		Hist->qtq_vs_ich = Dir->make<TProfile>(
		    "qtq_vs_ich",
		    Form("run %06i: link %02i:%i:%i Qt/Q vs ich", RunNumber, Station, Dtc, Link),
		    100,
		    0.,
		    100.,
		    0,
...
//-----------------------------------------------------------------------------
void TrackerDQM::book_event_histograms(art::TFileDirectory* Dir,
                                       int                  RunNumber,
                                       EventHist_t*         Hist)
{
	Hist->nhits = Dir->make<TH1F>(
	    "nhits", Form("run %06i: nhits total", RunNumber), 1000, 0., 5000.);
	Hist->nbtot = Dir->make<TH1F>(
	    "nbtot", Form("run %06i: nbytes total", RunNumber), 1000, 0., 100000.);
	Hist->nfrag =
	    Dir->make<TH1F>("nfrag", Form("run %06i: n fragments", RunNumber), 100, 0., 100.);
	Hist->fsize = Dir->make<TH1F>(
	    "fsize", Form("run %06i: fragment size", RunNumber), 1000, 0., 100000.);
	Hist->valid =
	    Dir->make<TH1F>("valid", Form("run %06i: valid code", RunNumber), 100, 0., 100.);
	...
	Hist->error_code = Dir->make<TH1F>(
	    "error_code", Form("run %06i: error code", RunNumber), 512, 0., 512.);
	Hist->nerr_tot = Dir->make<TH1F>(
	    "nerr_tot", Form("run %06i: N errors", RunNumber), 1000, 0., 2000.);
	Hist->nerr_vs_evt = Dir->make<TH1F>(
	    "nerr_vs_evt", Form("run %06i: N err vs evt", RunNumber), 1000, 0., 5000000.);
	Hist->eflg_vs_evt = Dir->make<TH1F>(
	    "eflg_vs_evt", Form("run %06i: eflag vs evt", RunNumber), 1000, 0., 5000000.);
}
...
void TrackerDQM::book_histograms(int RunNumber)
{
	...
	for(int i = 0; i < kNStationHistSets; i++)
		book_station_histset[i] = 0;
	...
	for(int ist = 0; ist < kNStationHistSets; ist++)
	{
		if(book_station_histset[ist] != 0)
		{
			...
			for(int idtc = 0; idtc < 2; idtc++)
			{
				...
				book_dtc_histograms(
				    &dtc_dir, RunNumber, &_hist.station[ist]->dtc[idtc], ist, idtc);
				for(int i = 0; i < _nActiveLinks[idtc]; i++)
				{
					...
					book_roc_histograms(&roc_dir,
					                    RunNumber,
					                    &_hist.station[ist]->dtc[idtc].roc[link],
					                    ist,
					                    idtc,
					                    link);
				}
		...
		for(int ih = 0; ih < nh; ih++)
		{
			...
			if(_fillWfHistograms)
			{
				...
				else
				{
					...
					{
						TLOG(TLVL_WARNING)
						    << "DTC_ID:" << Rd->dtc_id << " link:" << Rd->link
						    << " ih=" << ih << " ch:0x" << std::hex << ich << " hits > "
						    << std::dec << nmax
						    << " do not make extra waveform histograms" << std::endl;
					}
				}
				//-----------------------------------------------------------------------------
...
int TrackerDQM::fill_station_histograms(StationHist_t* Hist, EventData_t* Data)
{
	for(int idtc = 0; idtc < 2; idtc++)
	{
		for(int ir = 0; ir < _nActiveLinks[idtc]; ir++)
		{
			...
			fill_roc_histograms(&Hist->dtc[idtc].roc[link],
			                    &Data->station[0].roc[idtc][link]);
		}
	}
...
//-----------------------------------------------------------------------------
void TrackerDQM::analyze_dtc_fragment(const art::Event&       Evt,
                                      const artdaq::Fragment* Fragment)
{
	...
	if(fdata[0] > _maxFragmentSize)
	{
		...
		if(DebugBit(0) != 0)
		{
			TLOG(TLVL_ERROR) << Form(
			    "event %6i:%8i:%8i : ERROR_CODE:0x%04x in %s: fdt->nbytes= %i, BAIL "
			    "OUT\n",
			    Evt.run(),
			    Evt.subRun(),
			    Evt.event(),
	...
	short* first_address =
	    fdata + sizeof(SubEventHeader_t) /
	                2;  // _dataHeaderOffset; // offset is specified in 2-byte words
	...
	while(first_address < last_address)
	{
		...
		if(found == 0)
		{
			...
			{
				TLOG(TLVL_ERROR) << Form(
				    "event %6i:%8i:%8i : ERROR_CODE:0x%04x in %s: link=%i not defined as "
				    "active, BAIL OUT\n",
				    Evt.run(),
				    Evt.subRun(),
				    Evt.event(),
...
void TrackerDQM::analyze(const art::Event& AnEvent)
{
	...
	{
		//-----------------------------------------------------------------------------
		// different fragments correspond to different DTCs, somewhere there should be the
		// DTC ID
		//-----------------------------------------------------------------------------
		...
		if(nbytes < 2)
		{
			...
			TLOG(TLVL_DEBUG) << Form(
			    "event %i:%i:%i : ERROR_CODE:0x%04x nbytes=%i EMPTY_FRAGMENT",
			    AnEvent.run(),
			    AnEvent.subRun(),
			    AnEvent.event(),
			    kNBytesErrorBit,
		...
		{
			printf("%s\n",
			       Form("---------- fragment # %3i nbytes: %5i fsize: %5i ERROR_CODE: "
			            "0x%04x\n",
			            ifrag,
			            nbytes,
			            fsize,
		...
		//    if ((_edata.error == 0) and _analyzeFragments)
		//    analyze_fragment(event,&frag);
		if(_analyzeFragments)
			analyze_dtc_fragment(AnEvent, &frag);
		ifrag++;
	}
	//-----------------------------------------------------------------------------
	...
	if(_diagLevel > 1)
	{
		...
		{
			TLOG(TLVL_DEBUG) << Form(
			    " Run : %5i subrun: %5i event: %8i nfrag: %3i nbytes: %5i\n",
			    AnEvent.run(),
			    AnEvent.subRun(),
			    AnEvent.event(),
			    _edata.nfrag,
...
void TrackerDQM::debug(const art::Event& AnEvent)
{
	...
	for(const artdaq::Fragment& frag : *handle)
	{
		...
		{
			print_message(
			    Form("bit:000: fragment # %3i dtc_index:%i nbytes: %5i fsize: %5i "
			         "ERROR_CODE: 0x%04x NERR_TOT: %5i",
			         ifrag,
			         dtc_index,
			         nbytes,
			         fsize,
		...
		{
			print_message(
			    Form("bit:003: fragment # %3i dtc_index:%i nnbytes: %5i fsize: %5i "
			         "ERROR_CODE: 0x%04x NERR_TOT: %5i",
			         ifrag,
			         dtc_index,
			         nbytes,
...
void TrackerDQM::analyze_roc_data(RocDataHeaderPacket_t* Dh, RocData_t* Rd)
{
	...
	for(int ihit = 0; ihit < Rd->nhits; ihit++)
	{
		...
		hit =
		    (mu2e::TrackerDataDecoder::TrackerDataPacket*)(first_address + 0x08 + offset);
		...
		{   // 4 bits
			...
			{
				TLOG(TLVL_ERROR)
				    << "dtc_id:" << Rd->dtc_id << " link:" << Rd->link << " ih=" << ihit
				    << " first_address:0x" << std::hex << first_address
				    << " offset(bytes):0x" << std::hex << offset_in_bytes
				    << " HIT ERROR FLAG:0x" << std::hex << hit->ErrorFlags << std::endl;
			}
		...
		if(hit->NumADCPackets != _nADCPackets)
		{
			...
			{
				TLOG(TLVL_ERROR)
				    << "DTC:" << Rd->dtc_id << " link:" << Rd->link << " ih=" << ihit
				    << " first_address:0x" << std::hex << first_address << " offset:0x"
				    << std::hex << offset_in_bytes
		...
		if(ich > 95)
		{
			...
			if(DebugBit(0) != 0)
			{
				TLOG(TLVL_ERROR) << Form("event %i:%i:%i : ",
				                         _edata._event->run(),
				                         _edata._event->subRun(),
				                         _edata._event->event())
				                 << Form(
				                        " ERROR_CODE:0x%04x : link = %i offset(bytes): "
				                        "0x%04x hit->StrawIndex = 0x%04x\n",
				                        kChIDErrorBit,
				                        link,
				                        offset_in_bytes,
				                        hit->StrawIndex);
		...
		if(_validateADCPatterns >= 0)
		{
			...
			if(offs == -1)
			{
				TLOG(TLVL_ERROR)
				    << Form("event %i:%i:%i",
				            ...
				            _edata._event->subRun(),
				            _edata._event->event())
				    << Form(", : WRONG ADC PATTERN 0x%04x in %s: ",
				            kAdcPatternErrorBit,
				            __func__)
			...
			{
				//-----------------------------------------------------------------------------
				// starting pattern found, validating - the first word (#0) already
				// checked, thus starting from 1
				//-----------------------------------------------------------------------------
				for(int i = 1; i < nw; i++)
				{
					int ipat = (offs + i) % 4;
					if(w[loc + i] == pattern[ipat])
						continue;
					//-----------------------------------------------------------------------------
					// in trouble
					//-----------------------------------------------------------------------------
					TLOG(TLVL_ERROR) << Form("event %i:%i:%i",
					                 ...
					                 << Form(", : WRONG ADC PATTERN 0x%04x in %s: ",
					                         ...
					                         __func__)
					                 << Form(
					                        " link = %i offset(bytes): 0x%04x "
					                        "hit->StrawIndex = 0x%04x\n",
					                        link,
					                        offset_in_bytes,
					                        hit->StrawIndex);
	...
	for(int i = 0; i < kNChannels; i++)
	{
		...
		if((nhr > 0) and (nh > 0))
		{
			...
			chd->dt0r_c = chd->dt0r;
			if(chd->dt0r > dt_over_2 / 2)
				chd->dt0r_c = chd->dt0r + _gen_offset[i] - _dt;
			if(chd->dt0r < -dt_over_2 / 2)
				chd->dt0r_c = chd->dt0r + _gen_offset[i];

			chd->dt1r = (t1 - t1r) * _tdc_bin_ns;  // convert to ns

			chd->dt1r_c = chd->dt1r;
			if(chd->dt1r > dt_over_2 / 2)
				chd->dt1r_c = chd->dt1r + _gen_offset[i] - _dt;
			if(chd->dt1r < -dt_over_2 / 2)
				chd->dt1r_c = chd->dt1r + _gen_offset[i];
		}
	}
	//-----------------------------------------------------------------------------
