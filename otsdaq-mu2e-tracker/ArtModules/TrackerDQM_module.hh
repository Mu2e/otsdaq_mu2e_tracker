...
class TrackerDQM : public art::EDAnalyzer
{
	struct Config
	{
		...
		fhicl::Atom<art::InputTag> trkfCollTag{Name("trkfCollTag"),
		                                       Comment("track fragment coll tag")};
		fhicl::Atom<int>           minNBytes{Name("minNBytes"), Comment("min N(bytes)")};
		fhicl::Atom<int>           maxNBytes{Name("maxNBytes"), Comment("max N(bytes)")};
		fhicl::Sequence<int>       activeLinks_0{Name("activeLinks_0"),
                                           Comment("DTC0 active Links")};
		fhicl::Sequence<int>       activeLinks_1{Name("activeLinks_1"),
                                           Comment("DTC1 active Links")};
		fhicl::Sequence<int>       refChCal{Name("refChCal"),
                                      Comment("reference channel CAL side")};
		fhicl::Sequence<int>       refChHV{Name("refChHV"),
                                     Comment("reference channel HV  side")};
		fhicl::Atom<int>           dumpDTCRegisters{Name("dumpDTCRegisters"),
                                          Comment("1:dumpDTCRegisters")};
		fhicl::Atom<int>           analyzeFragments{Name("analyzeFragments"),
                                          Comment("1:analyzeFragments")};
		fhicl::Atom<int>           maxFragmentSize{Name("maxFragmentSize"),
                                         Comment("max fragment size")};
		fhicl::Atom<int>           pulserFrequency{Name("pulserFrequency"),
                                         Comment("pulser frequency")};
		fhicl::Atom<int> nADCPackets{Name("nADCPackets"), Comment("N(ADC packets/hit)")};
		fhicl::Atom<int> timeWindow{Name("timeWindow"),
		                            Comment("time window, 25 ns ticks")};
		fhicl::Atom<int> nSamplesBL{Name("nSamplesBL"), Comment("N(baseline samples)")};
		fhicl::Atom<float>   minPulseHeight{Name("minPulseHeight"),
                                          Comment("min pulse over threshold")};
		fhicl::Atom<float>   minNErrors{Name("minNErrors"),
                                      Comment("min N(errors) to print")};
		fhicl::Atom<float>   errorCode{Name("errorCode"), Comment("error code to print")};
		fhicl::Atom<float>   validateADCPatterns{Name("validateADCPatterns"),
                                               Comment("validate ADC patterns")};
		fhicl::Atom<float>   fillHistograms{Name("fillHistograms"),
                                          Comment("1:fill histograms")};
		fhicl::Atom<float>   fillWfHistograms{Name("fillWfHistograms"),
                                            Comment("1:fill WF histograms")};
		fhicl::Atom<float>   interactiveMode{Name("interactiveMode"),
                                           Comment("1:interactive mode")};
		fhicl::Sequence<int> plotWaveforms{Name("plotWaveforms"),
		                                   Comment("[link, channel]")};
	...
	struct DtcDataHeaderPacket_t : public DtcDMAPacket_t
	...
	int _adc_index_1[kNChannels];  // fixed map, seq num of the channel 'i' in the readout
	                               // sequence
  ...
  public:
	...
	void book_channel_histograms(
	    art::TFileDirectory* Dir, int RunNumber, ChannelHist_t* Hist, int Link, int Ich);
	void book_event_histograms(art::TFileDirectory* Dir,
	                           int                  RunNumber,
	                           EventHist_t*         Hist);
	void book_dtc_histograms(
	    art::TFileDirectory* Dir, int RunNumber, DtcHist_t* Hist, int IStation, int IDtc);
	void book_roc_histograms(art::TFileDirectory* Dir,
	                         int                  RunNumber,
	                         RocHist_t*           Hist,
	                         int                  IStation,
	                         int                  IDtc,
	...
	void unpack_adc_waveform(mu2e::TrackerDataDecoder::TrackerDataPacket* Hit,
	                         float*                                       Wf,
	                         WfParam_t*                                   Wp);
};
