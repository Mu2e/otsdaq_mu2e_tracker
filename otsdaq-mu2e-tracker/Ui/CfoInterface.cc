...
// most repeats Ryan's code, but w/o dependency on xdaq, GUI-based UI, and otsdaq
// everything
//-----------------------------------------------------------------------------
...
CfoInterface::CfoInterface(int PcieAddr, uint DtcMask, DTC_SimMode SimMode, bool SkipInit)
{
	...
	for(int i = 0; i < 8; i++)
	{
		...
		if(ndtcs > 0)
			...
			fCfo->DisableLink(CFO_Link_ID(i), DTC_LinkEnableMode(false, false));
	}
}
...
CfoInterface* CfoInterface::Instance(int PcieAddr, uint DtcMask)
{
	...
	if(pcie_addr < 0)
	{
		...
		//-----------------------------------------------------------------------------
		if(getenv("CFOLIB_CFO") != nullptr)
			pcie_addr = atoi(getenv("CFOLIB_CFO"));
		else
		{
			TLOG(TLVL_ERROR) << Form(
			    "PcieAddr < 0 and $CFOLIB_CFO is not defined. BAIL out\n");
			return nullptr;
		}
	}

	if(fgInstance == nullptr)
		fgInstance = new CfoInterface(pcie_addr, DtcMask, DTC_SimMode_NoCFO);

	if(fgInstance->PcieAddr() != pcie_addr)
	{
		TLOG(TLVL_ERROR) << Form(
		    "CfoInterface::Instance has been already initialized with PcieAddress = %i. "
		    "BAIL out\n",
		    fgInstance->PcieAddr());
		return nullptr;
	}
...
int CfoInterface::ConfigureJA(int ClockSource, int Reset)
{
	...
	if(ok == 0)
		TLOG(TLVL_ERROR) << "failed to setup CFO JA\n" << std::endl;
...
//   cfo_i->Cfo()->DisableLinks();                                    // Ryan says this is
//   important cfo_i->Cfo()->DisableEmbeddedClockMarker(); cfo_i->Cfo()->DisableBeamOnMode
//   (CFO_Link_ID::CFO_Link_ALL);    //
