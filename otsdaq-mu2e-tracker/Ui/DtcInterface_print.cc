...
void DtcInterface::PrintRocRegister(uint Reg, std::string& Desc, int Format, int LinkMask)
{
	...
	if(Format == 1)
		text += Form(" %s", Desc.data());
	cout << Form("%-18s %s\n", sreg.data(), text.data());
}

//-----------------------------------------------------------------------------
void DtcInterface::PrintRocRegister2(uint         Reg,
                                     std::string& Desc,
                                     int          Format,
                                     int          LinkMask)
{
...
void DtcInterface::PrintRocStatus(int Format, int LinkMask)
{
	...
	reg  = 8;
	desc = "ROC pattern mode ??";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 15;
	desc = "N simulated hits per lane";
	PrintRocRegister(reg, desc, Format, link_mask);
	...
	reg  = 7;
	desc = "Fiber loss/lock counter";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 6;
	desc = "Bad Markers counter";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 4;
	desc = "Loopback coarse delay";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 23;
	desc = "SIZE_FIFO_FULL [28]+STORE_POS[25:24]+STORE_CNT[19:0]";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 25;
	desc = "SIZE_FIFO_EMPTY[28]+FETCH_POS[25:24]+FETCH_CNT[19:0]";
	PrintRocRegister2(reg, desc, Format, link_mask);
	...
	reg  = 33;
	desc = "Num PREFETCH seen";
	PrintRocRegister2(reg, desc, Format, link_mask);

	cout << Form("\n");

	reg  = 9;
	desc = "Num DATA REQ seen";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 35;
	desc = "Num DATA REQ written to DDR";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 13;
	desc = "Num skipped DATA REQ";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 37;
	desc = "Num DATA REQ read from DDR";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 39;
	desc = "Num DATA REQ sent to DTC";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 41;
	desc = "Num DATA REQ with null data";
	PrintRocRegister2(reg, desc, Format, link_mask);
	...
	reg  = 48;
	desc = "Last PREFETCH tag";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 51;
	desc = "Last fetched tag";
	PrintRocRegister2(reg, desc, Format, link_mask);

	reg  = 54;
	desc = "Last DATA REQ tag";
	PrintRocRegister2(reg, desc, Format, link_mask);
	...
	reg  = 72;
	desc = "Num HB tag inconsistencies";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 73;
	desc = "Num DATA REQ tag inconsistencies";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 74;
	desc = "Num HB tag lost";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 75;
	desc = "Num DATA REQ tag lost";
	PrintRocRegister(reg, desc, Format, link_mask);

	reg  = 0x90;
	desc = "total N packets (DCS+data)";             // r_144
	PrintRocRegister(reg, desc, Format, link_mask);  //
	reg  = 0x91;
	desc = "N(DCS) packets sent to DTC";             // r_145
	PrintRocRegister(reg, desc, Format, link_mask);  //
	reg  = 0x92;
	desc = "Num of non-DCS packets";                 // r_146
	PrintRocRegister(reg, desc, Format, link_mask);  //
	reg  = 0x93;
	desc = "Num of data header packets";             // r_147
	PrintRocRegister(reg, desc, Format, link_mask);  //
	reg  = 0x94;
	desc = "Num of data payload packets";            // r_148
	PrintRocRegister(reg, desc, Format, link_mask);  //
	reg  = 0x95;
	desc = "Num of empty data packets";              // r_149
	PrintRocRegister(reg, desc, Format, link_mask);  //
...
void DtcInterface::PrintStatus()
{
	cout << Form("-----------------------------------------------------------------\n");
	cout << Form(
	    " PCIE address: %i link mask: 0x%04x SampleEdgeMode: %i RocReadoutMode: %i\n",
	    fPcieAddr,
	    fLinkMask,
	    fSampleEdgeMode,
	    fRocReadoutMode);
