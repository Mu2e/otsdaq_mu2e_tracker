...
struct ControlRoc_Read_Input_t
{
	...
	uint16_t
	    num_triggers[2];  // num_triggers = (dtcbuffer[4] << 16) + dtcbuffer[3]; // -T
	uint16_t
	    ch_mask[6];  // channel_mask[0] = (dtcbuffer[6] << 16) + dtcbuffer[5];  // -C
	...
	// uint16_t    adc_mode;               // -a 8   (defailt:  0)  [0] // *v2* uint16_t
	// tdc_mode;               // -t 8   (default:  0)  [1]                   // *v2*
	// uint16_t    num_lookback;           // -l 8   (default:  8)  [2] // *v2* uint16_t
	// num_samples;            // -s 1   (default: 16)  [3] if>63, set to 63  // *v2*
	// uint16_t    num_triggers[2];        // -T 10  (default:  0)  [4-5] // *v2* uint16_t
	// ch_mask[6];             // FFFF FFFF FFFF FFFF FFFF FFFF [6:11]        // *v2*
	// uint16_t    enable_pulser;          // -p 1     (default: 0) [12] // *v2* uint16_t
	// marker_clock;           // -m 3 ??? )default: 0) [13]                  // *v2*
	// uint16_t    mode;                   // [14] need to set mode=0 // *v2* uint16_t
	// clock;                  // [15] need to set clock=99                   // *v2*
#endif
};
...
struct ControlRoc_DigiRW_Input_t
{
	...
	uint16_t
	    data[2];  // num_triggers = (dtcbuffer[4] << 16) + dtcbuffer[3];         // -T
};
