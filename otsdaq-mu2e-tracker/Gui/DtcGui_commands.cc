...
void DtcGui::cfo_print_status(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	...
	{
		*TextView << Form("ERROR in DtcGui::%s: coudn't read the CFO\n", __func__);
	}
}
...
void DtcGui::print_dtc_status()
{
	...
	TDatime x1;
	*fTextView << x1.AsSQLString() << Form(" DtcGui::%s:START:\n", __func__);
	...
	TDatime x2;
	*fTextView << strCout.str() << x2.AsSQLString()
	           << Form(" DtcGui::%s:DONE\n", __func__);
...
void DtcGui::cfo_init_readout(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	...
	try
	{
		...
		std::string run_plan = Form("%s/test_stand/cfo_run_plans/%s",
		                            gSystem->Getenv("HOME"),
		                            Dtel->fRunPlan->GetText());

		TLOG(TLVL_INFO) << Form(
		    "run_plan, dtc_mask: %s 0x%08x\n", run_plan.data(), dtc_mask);
...
void DtcGui::cfo_disable_beam_off()
{
	...
	*fTextView << x1.AsSQLString() << "DtcGui::" << __func__ << ": START" << std::endl;
	...
	try
	{
		fCFO_i->Cfo()->DisableBeamOffMode(CFO_Link_ID::CFO_Link_ALL);
	}
	catch(...)
	{
		*fTextView << Form("ERROR : DtcGui::cfo_disable_beam_off failed\n");
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form("DtcGui::%s: DONE\n", __func__);
	fTextView->ShowBottom();
	// restore cout
	cout.rdbuf(oldCoutStreamBuf);
...
void DtcGui::cfo_enable_beam_off()
{
	...
	*fTextView << x1.AsSQLString() << "DtcGui::" << __func__ << ": START" << std::endl;
	...
	try
	{
		fCFO_i->Cfo()->EnableBeamOffMode(CFO_Link_ID::CFO_Link_ALL);
	}
	catch(...)
	{
		*fTextView << Form("ERROR : DtcGui::cfo_enable_beam_off failed\n");
	}
...
void DtcGui::cfo_launch_run_plan()
{
	...
	*fTextView << x1.AsSQLString() << Form("DtcGui::%s: START\n", __func__);
	...
	try
	{
		fCFO_i->LaunchRunPlan();
	}
	catch(...)
	{
		*fTextView << Form("ERROR : coudn't launch run plan... BAIL OUT") << std::endl;
	}
...
void DtcGui::cfo_set_ja_mode()
{
	...
	{
		*fTextView << Form("ERROR in DtcGui::%s: failed to execute", __func__);
	}
}
...
void DtcGui::configure_roc_pattern_mode()
{
	...
	TDatime t1;
	*fTextView << t1.AsSQLString() << " DtcGui::" << __func__ << ": START" << std::endl;

	DtcTabElement_t* dtel = fDtcTel + fActiveDtcID;
	dtel->fDTC_i->RocConfigurePatternMode();

	TDatime t2;
	*fTextView << t1.AsSQLString() << " DtcGui::" << __func__ << ": DONE" << std::endl;
	fTextView->ShowBottom();
	// Restore old cout.
	cout.rdbuf(oldCoutStreamBuf);
}
...
//-----------------------------------------------------------------------------
void DtcGui::exit() { gApplication->Terminate(); }
...
void DtcGui::dtc_print_firefly_temp(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	try
	{
		Dtel->fDTC_i->PrintFireflyTemp();
	}
	catch(...)
	{
		*fTextView << Form("ERROR, BAIL OUT") << std::endl;
	}
}
...
void DtcGui::dtc_print_all_rocs(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	try
	{
		Dtel->fDTC_i->PrintRocStatus(1, -1);
	}
	catch(...)
	{
		*fTextView << Form("ERROR, BAIL OUT") << std::endl;
...
void DtcGui::print_roc_status()
{
	...
	*fTextView << x1.AsSQLString() << "DtcGui:: : " << __func__ << ": START" << std::endl;
	...
	try
	{
		dtel->fDTC_i->PrintRocStatus(1, 1 << 4 * roc);
	}
	catch(...)
	{
		*fTextView << Form("ERROR : coudn't read ROC %i ... BAIL OUT", roc) << std::endl;
	}
...
void DtcGui::dtc_init_readout(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	...
	{
		*TextView << Form(
		    "ERROR in DtcGui::%s: failed to execute DtcInterface::InitReadout\n",
		    __func__);
	}
}
...
// init (preconfigured) readout
//-----------------------------------------------------------------------------
void DtcGui::dtc_launch_run_plan_emulated_cfo(DtcTabElement_t*   Dtel,
                                              TGTextViewostream* TextView)
{
	...
	{
		*TextView << Form(
		    "ERROR in DtcGui::%s: failed to execute "
		    "DtcInterface::LaunchRunPlanEmulatedCfo\n",
		    __func__);
	}
}
...
void DtcGui::dtc_soft_reset(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	...
	{
		*TextView << "ERROR : coudn't soft reset DTC ... BAIL OUT" << std::endl;
	}
}
...
void DtcGui::dtc_hard_reset(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	...
	{
		*TextView << Form("ERROR : coudn't soft reset DTC ... BAIL OUT\n");
	}
}
...
void DtcGui::dtc_read_subevents(DtcTabElement_t* Dtel, TGTextViewostream* TextView)
{
	try
	{
		...
		Dtel->fDTC_i->ReadSubevents(
		    list_of_dtc_blocks, first_ewm, print_level, validate, output_fn);
	}
	catch(...)
	{
		*TextView << Form("ERROR : coudn't read the DTC ... BAIL OUT\n");
	}
}
...
void DtcGui::init_external_cfo_readout_mode()
{
	...
	TDatime x1;
	*fTextView << x1.AsSQLString() << Form(" %s: DTC ID: %i \n", __func__, fActiveDtcID);
	//-----------------------------------------------------------------------------
	...
	if(dtel->fData->fName == "DTC")
	{
		...
		{
			*fTextView << Form("ERROR : filed  BAIL OUT") << std::endl;
		}
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str() << Form(" %s: DONE", __func__)
	           << std::endl;
	fTextView->ShowBottom();
	// Restore old cout.
	cout.rdbuf(oldCoutStreamBuf);
...
void DtcGui::read_dtc_register()
{
	...
	*fTextView << x1.AsSQLString() << " DtcGui::" << __func__
	...
	if(dtel->fData->fName == "DTC")
	{
		...
		{
			*fTextView << Form(
			    "%s: reading DTC %i register 0x%08x\n", __func__, fActiveDtcID, reg);
		...
		{
			*fTextView << Form("ERROR in %s: coudn't read DTC reg 0x%4x... BAIL OUT",
			                   __func__,
			                   reg)
			           << std::endl;
		}
	...
	else if(dtel->fData->fName == "CFO")
	{
		...
		{
			*fTextView << Form(
			    "%s: reading CFO %i register 0x%08x", __func__, fActiveDtcID, reg);
		...
		{
			*fTextView << Form("ERROR in %s: coudn't read CFO reg 0x%4x... BAIL OUT",
			                   __func__,
			                   reg)
			           << std::endl;
		}
...
void DtcGui::read_roc_register()
{
	...
	*fTextView << x1.AsSQLString() << " DtcGui::" << __func__ << ": cmd: "
	           << "read_roc_register" << std::endl;

	if(fDebugLevel > 1)
		printf("DtcGui::%s : Active DTC ID: %i\n", __func__, fActiveDtcID);
	...
	if(dtel->fData->fName == "DTC")
	{
		...
		try
		{
			int      timeout_ms(150);
			uint16_t val =
			    dtel->fDTC_i->Dtc()->ReadROCRegister(DTC_Link_ID(roc), reg, timeout_ms);
			rtel->fValR->SetText(Form("0x%04x", val));
			*fTextView
			    << Form("%s: roc: %i reg : 0x%04x val: 0x%04x", __func__, roc, reg, val)
			    << std::endl;
		}
		catch(...)
		{
			*fTextView << Form("ERROR in %s: coudn't read ROC %i ... BAIL OUT",
			                   __func__,
			                   roc)
			           << std::endl;
		}
...
void DtcGui::read_subevents()
{
	...
	*fTextView << x1.AsSQLString() << " DtcGui::" << __func__ << std::endl;
	...
	if(dtel->fData->fName == "DTC")
	{
		...
		{
			*fTextView << Form("ERROR in %s: coudn't read event BAIL OUT", __func__)
			           << std::endl;
		}
	}
	...
	cout.rdbuf(str_cout.rdbuf());
	TDatime x2;
	*fTextView << x2.AsSQLString() << str_cout.str() << " DtcGui::" << __func__
	           << " : DONE " << std::endl;
	fTextView->ShowBottom();
	// Restore old cout.
	cout.rdbuf(old_cout_sb);
...
void DtcGui::reset_roc()
{
	...
	TDatime x1;
	*fTextView << x1.AsSQLString()
	           << Form(" %s: DTC ID: %i roc: %i\n", __func__, fActiveDtcID, roc);
	//-----------------------------------------------------------------------------
	// CFO doesn't have ROC's
	//-----------------------------------------------------------------------------
	if(dtel->fData->fName == "DTC")
	{
		...
		try
		{
			dtel->fDTC_i->ResetRoc(mask);
		}
		catch(...)
		{
			*fTextView << Form("ERROR : coudn't reset ROC %i ... BAIL OUT", roc)
			           << std::endl;
		}
	}
...
void DtcGui::set_emulate_cfo()
{
	...
	*fTextView << x1.AsSQLString() << Form(" %s: DTC ID: %i\n", __func__, fActiveDtcID);
	...
	{
		*fTextView << Form("ERROR : coudn't set emulate CFO flag... BAIL OUT\n");
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form(" %s DONE, emulate_cfo = %i\n", __func__, emulate_cfo);
	fTextView->ShowBottom();
	cout.rdbuf(oldCoutStreamBuf);
}
...
void DtcGui::set_ew_length()
{
	...
	*fTextView << x1.AsSQLString()
	...
	{
		*fTextView << Form("ERROR : coudn't read EW Length ... BAIL OUT") << std::endl;
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form(" %s DONE ew_length = %10i\n", __func__, ew_length);
	fTextView->ShowBottom();
	cout.rdbuf(oldCoutStreamBuf);
}
...
void DtcGui::set_nevents()
{
	...
	*fTextView << x1.AsSQLString()
	...
	{
		*fTextView << Form("ERROR : coudn't reset ROC %i ... BAIL OUT", roc) << std::endl;
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form(" %s DONE nevents = %10i\n", __func__, nevents);
	fTextView->ShowBottom();
	cout.rdbuf(oldCoutStreamBuf);
}
...
void DtcGui::set_print_freq()
{
	...
	*fTextView << x1.AsSQLString()
	...
	try
	{
		value = fPrintFreq->GetIntNumber();
	}
	catch(...)
	{
		*fTextView << Form("ERROR : coudn't set PrintFreq %i ... BAIL OUT", roc)
		           << std::endl;
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form(" %s DONE PrintFreq = %10i\n", __func__, value);
	fTextView->ShowBottom();
	cout.rdbuf(oldCoutStreamBuf);
}
...
void DtcGui::set_roc_readout_mode()
{
	...
	*fTextView << x1.AsSQLString() << Form(" %s: DTC ID: %i\n", __func__, fActiveDtcID);
	...
	{
		*fTextView << Form("ERROR : coudn't set ROC readout mode... BAIL OUT\n");
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form(" %s DONE, roc_readout_mode = %i\n", __func__, roc_readout_mode);
	fTextView->ShowBottom();
	cout.rdbuf(oldCoutStreamBuf);
}
...
void DtcGui::set_sleep_us()
{
	...
	*fTextView << x1.AsSQLString()
	...
	try
	{
		value = fSleepUS->GetIntNumber();
	}
	catch(...)
	{
		*fTextView << Form("ERROR : coudn't set SleepUS %i ... BAIL OUT", roc)
		           << std::endl;
	}

	TDatime x2;
	*fTextView << x2.AsSQLString() << strCout.str()
	           << Form(" %s DONE SleepUS = %10i\n", __func__, value);
	fTextView->ShowBottom();
	cout.rdbuf(oldCoutStreamBuf);
}
	...
	*fTextView << x1.AsSQLString() << " DtcGui::" << __func__ << ": cmd: "
	...
	if(dtel->fData->fName == "DTC")
	{
		try
		{
			...
			*fTextView << Form("%s: DTC %i reg 0x%8x val: 0x%8x",
			                   __func__,
			                   fActiveDtcID,
			                   reg,
			                   val)
		...
		{
			*fTextView << Form("ERROR in %s: coudn't write DTC reg 0x%4x... BAIL OUT",
			                   __func__,
			                   reg)
			           << std::endl;
		}
	...
	else if(dtel->fData->fName == "CFO")
	{
		try
		{
			...
			*fTextView << Form("%s: CFO %i reg 0x%8x val: 0x%8x",
			                   __func__,
			                   fActiveDtcID,
			                   reg,
			                   val)
		...
		{
			*fTextView << Form("ERROR in %s: coudn't write CFO reg 0x%4x... BAIL OUT",
			                   __func__,
			                   reg)
			           << std::endl;
		}
...
void DtcGui::write_roc_register()
{
	...
	TDatime x1;
	*fTextView << x1.AsSQLString() << " DtcGui::" << __func__ << " DTC: " << fActiveDtcID
	           << " ROC:" << roc << " reg:0x" << std::hex << reg << " val:0x" << std::hex
	           << val << std::endl;
	...
	{
		int timeout_ms(150);
		dtel->fDTC_i->Dtc()->WriteROCRegister(
		    DTC_Link_ID(roc), reg, val, false, timeout_ms);
	}
	catch(...)
	{
		*fTextView << Form(
		                  "ERROR in %s: coudn't write ROC %i ... BAIL OUT", __func__, roc)
		           << std::endl;
	}
...
int DtcGui::execute_command()
{
	...
	TDatime x1;
	*fTextView << x1.AsSQLString() << Form(" DtcGui::%s\n", __func__);
	void (*fun)(DtcGui*, DtcTabElement_t*, TGTextViewostream*);
	fun = (void (*)(
	    DtcGui*, DtcGui::DtcTabElement_t*, TGTextViewostream*))btn->GetUserData();
	fun(this, dtel, fTextView);

	*fTextView << strCout.str();
	TDatime x2;
	*fTextView << x2.AsSQLString() << Form(" DtcGui::%s: FINISHED\n", __func__);
	fTextView->ShowBottom();
