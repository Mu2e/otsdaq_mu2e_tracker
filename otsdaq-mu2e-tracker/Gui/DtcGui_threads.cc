...
void* DtcGui::ReaderThread(void* Context)
{
	...
	if(tc->fPrintLevel > 0)
	{
		// print header
		cout << Form(
		    "   CPU T   Real T      event  DTC    EW Tag   nbytes   nbytes_tot  "
		    "------------- ROC status ----------------  nerr nerr_tot\n");
		cout << Form(
		    "----------------------------------------------------------------------------"
		    "-----------------------------------------------\n");
	}
	...
	while(tc->fStop == 0)
	{
		if(tc->fPause != 0)
		{
			TLOG(TLVL_DEBUG) << "thread alive, sleep for " << tc->fSleepTimeMs << "ms"
			                 << std::endl;
			gSystem->Sleep(tc->fSleepTimeMs);
			continue;
		}

		if(tc->fPrintLevel > 100)
			TLOG(TLVL_DEBUG) << Form(" emoe : %10lu\n", tstamp);
			...
			while(1)
			{
				...
				for(int i = 0; i < sz; i++)
				{
					...
					if(dtc_gui->fValidate)
					{
						...
						{
							nerr = dtc_i->ValidateVarPatterns((ushort*)data,
							                                  ew_tag,
							                                  &offset,
							                                  tc->fPrintLevel,
							                                  nerr_roc);
						...
						{
							nerr = dtc_i->ValidateDigiPatterns((ushort*)data,
							                                   ew_tag,
							                                   &offset,
							                                   tc->fPrintLevel,
							                                   nerr_roc);
						...
						{
							nerr = dtc_i->ValidateFixedPatterns((ushort*)data,
							                                    ew_tag,
							                                    &offset,
							                                    tc->fPrintLevel,
							                                    nerr_roc);
					...
					if(tstamp % print_freq == 0)
					{
						if(tc->fPrintLevel > 1)
						{
							...
							timer.Continue();
							cout << Form("%8.2f %8.2f %10lu  %1i  %10lu %6i %13li",
							             ct,
							             rt,
							             tstamp,
							             i,
							             ...
							             nbytes,
							             nbytes_tot)
							     << Form(" 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x",
							             rs[0],
							             rs[1],
							             rs[2],
							             rs[3],
							...
							{
								dtc_i->PrintBuffer(dtc_block->GetRawBufferPointer(),
								                   dtc_block->GetSubEventByteCount() / 2);
							}
						}
				...
				{
					//-----------------------------------------------------------------------------
					// sz === NDTCs = 0: zero length event - end-of-read- print only if
					// PrintLevel > 10 everything is read in, safe to release all buffers
					//-----------------------------------------------------------------------------
					//          TThread::Lock();
					...
					{
						cout << Form(
						    ">>>> ------- tstamp = %10lu event_tg:%10lu NDTCs:%2i\n",
						    tstamp,
						    event_tag.GetEventWindowTag(true),
						    sz);
					}
					break;
				}
...
void* DtcGui::EmuCfoThread(void* Context)
{
	...
	{
		TLOG(TLVL_ERROR) << "DtcInterface::fEmulateCfo = 0, cant run in emulated mode"
		                 << std::endl;
		return nullptr;
	}
	...
	TLOG(TLVL_DEBUG) << Form(
	    "%s: START ew_length: %5i nevents: %10lu first_tx:%10lu sleep_us:%8i\n",
	    __func__,
	    ew_length,
	    nevents,
	    first_ts,
...
void* DtcGui::ExtCfoThread(void* Context)
{
	...
	//-----------------------------------------------------------------------------
	// for now, assume that the run plans are stored in ~/test_stand/cfo_run_plans
	// directory
	//-----------------------------------------------------------------------------
	std::string run_plan =
	    Form("~/test_stand/cfo_run_plans/%s", dtel->fRunPlan->GetText());
	int dtc_mask;
