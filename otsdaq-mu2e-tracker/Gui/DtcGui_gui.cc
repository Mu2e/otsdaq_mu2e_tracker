...
DtcGui::DtcGui(const char* Project, const TGWindow* p, UInt_t w, UInt_t h, int DebugLevel)
{
	...
	// //-----------------------------------------------------------------------------
	// // there is only one CFO, so initialize the CFO pointer - by copying it from the
	// tab
	// //-----------------------------------------------------------------------------
	//       fCFO_i = dtel->fCFO_i;
	//     }
...
int DtcGui::InitRunConfiguration(const char* Config)
{
	...
	TString macro =
	    Form("%s/config/dtc_gui/%s.C", gSystem->Getenv("MU2E_DAQ_DIR"), Config);
	FILE* f = fopen(macro, "r");
	if(f == nullptr)
	{
		macro = Form("%s/config/dtc_gui/%s/%s.C",
		             gSystem->Getenv("MU2E_DAQ_DIR"),
		             Config,
		             fHostname.Data());
		f     = fopen(macro, "r");
		if(f == nullptr)
		{
			TLOG(TLVL_ERROR) << "failed to find config file for " << Config << " , EXIT"
			                 << std::endl;
			rc = -1;
		}
	}
...
void DtcGui::BuildGui(const TGWindow* Parent, UInt_t Width, UInt_t Height)
{
	...
	fMainFrame =
	    new TGMainFrame(gClient->GetRoot(),
	                    Width,
	                    Height,
	                    kMainFrame | kVerticalFrame);  // ,kMainFrame | kVerticalFrame);
	...
	for(int i = 0; i < fNDtcs; i++)
	{
		if(fDtcData[i].IsDtc())
			...
			BuildCfoTabElement(fDtcTab, fDtcTel[i], &fDtcData[i]);
	}

	fDtcTab->MoveResize(
	    10, 10, 920, 260);  // this defines the size of the tab below the tabs line
	...
	fMainFrame->AddFrame(fButtonsFrame,
	...
	// 1. clear
	//-----------------------------------------------------------------------------
	tb = new TGTextButton(fButtonsFrame,
	                      "clear",
	                      -1,
	...
	fNEvents = new TGNumberEntry(fButtonsFrame,
	                             66,
	                             9,
	                             999,
	...
	(fNEvents->GetNumberEntry())
	    ->Connect("ReturnPressed()", "DtcGui", this, "set_nevents()");

	fButtonsFrame->AddFrame(fNEvents,
	                        new TGLayoutHints(kLHintsLeft | kLHintsTop, 2, 2, 2, 2));
	...
	fEWLength = new TGNumberEntry(fButtonsFrame,
	                              1000,
	                              9,
	                              999,
	                              TGNumberFormat::kNESInteger,
	...
	(fEWLength->GetNumberEntry())
	    ->Connect("ReturnPressed()", "DtcGui", this, "set_ew_length()");

	fButtonsFrame->AddFrame(fEWLength,
	                        new TGLayoutHints(kLHintsLeft | kLHintsTop, 2, 2, 2, 2));
	//-----------------------------------------------------------------------------
	...
	fFirstTS = new TGNumberEntry(fButtonsFrame,
	                             0,
	                             9,
	                             999,
	                             TGNumberFormat::kNESInteger,
	...
	(fFirstTS->GetNumberEntry())
	    ->Connect("ReturnPressed()", "DtcGui", this, "set_first_ts()");

	fButtonsFrame->AddFrame(fFirstTS,
	                        new TGLayoutHints(kLHintsLeft | kLHintsTop, 2, 2, 2, 2));
	//-----------------------------------------------------------------------------
	...
	fSleepUS = new TGNumberEntry(fButtonsFrame,
	                             2000000,
	                             9,
	                             999,
	                             TGNumberFormat::kNESInteger,
	...
	(fSleepUS->GetNumberEntry())
	    ->Connect("ReturnPressed()", "DtcGui", this, "set_sleep_us()");

	fButtonsFrame->AddFrame(fSleepUS,
	                        new TGLayoutHints(kLHintsLeft | kLHintsTop, 2, 2, 2, 2));
	//-----------------------------------------------------------------------------
	...
	fPrintFreq = new TGNumberEntry(fButtonsFrame,
	                               1,
	                               9,
	                               999,
	                               TGNumberFormat::kNESInteger,
	...
	(fPrintFreq->GetNumberEntry())
	    ->Connect("ReturnPressed()", "DtcGui", this, "set_print_freq()");

	fButtonsFrame->AddFrame(fPrintFreq,
	                        new TGLayoutHints(kLHintsLeft | kLHintsTop, 2, 2, 2, 2));
	//-----------------------------------------------------------------------------
	// set active DTC tab
	//-----------------------------------------------------------------------------
