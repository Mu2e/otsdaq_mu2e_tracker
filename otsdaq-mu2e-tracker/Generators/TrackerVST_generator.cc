...

{
	...
	for(int i = 0; i < _nActiveLinks; i++)
	{
		...
		else if(_readoutMode == kReadPattern)
			monica_var_pattern_config(_dtc, _activeLinks[i]);
	}
	//-----------------------------------------------------------------------------
	...
	do
	{
		...
		//-----------------------------------------------------------------------------
		if(flag != 0)
			_dtc->GetDevice()->write_register(r, 100, 0xffffffff);
		_reg[_nreg] = r;
		_nreg += 1;
		i += 1;
...
int mu2e::TrackerVST::message(const std::string& msg_type, const std::string& message)
{
	...
	TLOG(TLVL_INFO) << "trying to send XML-RPC message ... msg_type: " << msg_type
	                << " message: " << message;
...
void mu2e::TrackerVST::print_dtc_registers(DTC* Dtc, const char* Header)
{
	...
	uint32_t res;
	int      rc;
	rc = _device->read_register(0x9100, 100, &res);
	printf("DTC status       : 0x%08x rc:%i\n", res, rc);  // expect: 0x40808404
	rc = _device->read_register(0x91c8, 100, &res);
	printf("debug packet type: 0x%08x rc:%i\n", res, rc);  // expect: 0x00000000
}
...
//-----------------------------------------------------------------------------
int mu2e::TrackerVST::readData(artdaq::Fragment* Frag)
{
	//-----------------------------------------------------------------------------
	...
	else
	{
		...
		message("alarm",
		        "TrackerVST::ReadData::ERROR event=" + std::to_string(ev_counter()) +
		            " nbytes=0");
	}
...
bool mu2e::TrackerVST::readEvent(artdaq::FragmentPtrs& Frags)
{
	...
	if((_debugLevel > 0) and (ev_counter() < _nEventsDbg))
	{
		...
		printf("DTC status       : 0x%08x rc:%i\n", res, rc);  // expect: 0x40808404
		rc = _dtc->GetDevice()->read_register(0x91c8, 100, &res);
		printf("debug packet type: 0x%08x rc:%i\n", res, rc);  // expect: 0x00000000
	}
...
//-----------------------------------------------------------------------------
bool mu2e::TrackerVST::simulateEvent(artdaq::FragmentPtrs& Frags)
{
	double            tstamp = ev_counter();
...
bool mu2e::TrackerVST::getNext_(artdaq::FragmentPtrs& Frags)
{
	...
	if(_networkCFO)
	{
		...
		int  cnt     = recvfrom(
            _sock, message, sizeof(message), 0, (struct sockaddr*)&_addr, &addrlen);
		if(cnt < 0)
		{
