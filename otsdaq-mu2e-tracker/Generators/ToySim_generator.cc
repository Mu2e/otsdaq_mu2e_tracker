...
demo::ToySim::ToySim(fhicl::ParameterSet const& ps)
    ...
    , fragment_type_(
          static_cast<decltype(fragment_type_)>(artdaq::Fragment::InvalidFragmentType))
    , distribution_type_(
          static_cast<ToyHW::DistributionType>(ps.get<int>("distribution_type")))
...

{
	...
	if(dies_on_config_)
	{
		TLOG(TLVL_ERROR) << "This is an engineered process death, set by the "
		                    "dies_on_config FHiCL variable";
		std::exit(1);
	}
	...
	TLOG(TLVL_INFO) << "Constructor: metadata_.unused = 0x" << std::hex
	                << metadata_.unused << " sizeof(metadata_) = " << std::dec
	                << sizeof(metadata_);
	...
	case 1003:
		...
		break;
	default:
		throw cet::exception("ToySim") << "Unable to determine board type supplied by "
		                                  "hardware";  // NOLINT(cert-err60-cpp)
	}
}
...
bool demo::ToySim::getNext_(artdaq::FragmentPtrs& frags)
{
	...
	// artdaq::Fragment::FragmentBytes(std::size_t payload_size_in_bytes, sequence_id_t
	// sequence_id,
	...
	for(auto& id : fragmentIDs())
	{
		...
		std::unique_ptr<artdaq::Fragment> fragptr(artdaq::Fragment::FragmentBytes(
		    bytes_read, ev_counter(), id, fragment_type_, metadata_, timestamp_));
		...
		TLOG(TLVL_DEBUG + 4) << "getNext_ after memcpy " << bytes_read
		                     << " bytes and std::move dataSizeBytes()="
		                     << frags.back()->sizeBytes()
	...
	if(metricMan != nullptr)
	{
		metricMan->sendMetric(
		    "Fragments Sent", ev_counter(), "Events", 3, artdaq::MetricMode::LastPoint);
	}
