...
std::vector<uint16_t> mu2e::TrackerBR::fragmentIDs()
{
	...
	if(_readDTCRegisters)
		v.push_back(FragmentType::TRKDTC);
...

{
	...
	//   HNDLE h_daq_host_conf       =
	//   odb_i->GetDaqHostHandle(hDB,h_active_run_conf,rpc_host);
	//
	//   HNDLE h_component;
	//   KEY   component;
	//   int   ncomp(0);
	//   int   pcie_addr(-1);
	//   for (int i=0; db_enum_key(hDB, h_daq_host_conf, i, &h_component) !=
	//   DB_NO_MORE_SUBKEYS; ++i) {
	//     db_get_key(hDB, h_component, &component);
	//     TLOG(TLVL_DBG+1) << "index: " << " Subkey:" <<  component.name << " Type: " <<
	//     component.type; if (component.name == _artdaqLabel) {
	// //-----------------------------------------------------------------------------
	// // the board reader configuration found, take the PCIE address from there
	// //-----------------------------------------------------------------------------
	//       int sz(0);
	//       if (db_get_value(hDB, h_component, "DTC/PCIEAddress", &pcie_addr, &sz,
	//       TID_INT32, TRUE) != DB_SUCCESS) {
	//         TLOG(TLVL_ERROR) << "no PCIE address for" << component.name << " in" <<
	//         active_run_conf ;
	//       }
	//     }
	//   }
...
// GetPartitionNumber() is an artdaq global function - see
// artdaq/artdaq/DAQdata/Globals.hh
//-----------------------------------------------------------------------------
int mu2e::TrackerBR::message(const std::string& msg_type, const std::string& message)
{
	auto _xmlrpcUrl = "http://" + _tfmHost + ":" +
	                  std::to_string((10000 + 1000 * GetPartitionNumber())) + "/RPC2";

	xmlrpc_client_call(&_env,
	                   _xmlrpcUrl.data(),
	                   "message",
	                   "(ss)",
	                   msg_type.data(),
	...
	{
		TLOG(TLVL_DBG + 1) << "message successfully sent. type:" << msg_type << " message"
		                   << message;
	}
	return 0;
}
...
int mu2e::TrackerBR::readData(artdaq::FragmentPtrs& Frags, ulong& TStamp)
{
	...
	TLOG(TLVL_DBG) << "------------- START TStamp=" << TStamp << std::endl;
	...
	try
	{
		...
		TLOG(TLVL_DBG) << "read =" << sz << " DTC blocks" << std::endl;
		if(sz > 0)
		{
			_tstamp += 1;
		}
		//-----------------------------------------------------------------------------
		// each subevent (a block of data corresponding to a single DTC) becomes an artdaq
		// fragment
		//-----------------------------------------------------------------------------
		for(int i = 0; i < sz; i++)
		{
			...
			if(nb > 0)
			{
				artdaq::Fragment* frag = new artdaq::Fragment(
				    ev_counter(), _fragment_ids[0], FragmentType::TRK, TStamp);
				...
				{
					TLOG(TLVL_INFO) << " subevent:" << i << " EW tag:" << ew_tag
					                << " nbytes: " << nb << std::endl;
					_dtc_i->PrintBuffer(ev->GetRawBufferPointer(),
			...
			else
			{
				...
				//-----------------------------------------------------------------------------
				TLOG(TLVL_ERROR)
				    << "zero length read, event:" << ev_counter() << std::endl;
				message("alarm",
				        "TrackerBR::ReadData::ERROR event=" +
				            std::to_string(ev_counter()) + " nbytes=0");
			}
		}
	...
	if(print_event)
	{
		TLOG(TLVL_DBG + 1) << "event readSuccess timeout: nbytes\n"
		                   << ev_counter() << " " << readSuccess << " " << timeout << " "
		                   << nbytes << std::endl;
	}
...
bool mu2e::TrackerBR::readEvent(artdaq::FragmentPtrs& Frags)
{
	...
	else
	{
		...
		// fake reading
		//-----------------------------------------------------------------------------
		artdaq::Fragment* f1 = new artdaq::Fragment(
		    ev_counter(), _fragment_ids[0], FragmentType::TRK, tstamp);
		f1->resizeBytes(4);
	...
	TLOG(TLVL_DBG) << "bufferes released, end" << std::endl;
	return true;
}
...
bool mu2e::TrackerBR::simulateEvent(artdaq::FragmentPtrs& Frags)
{
	...
	artdaq::Fragment* frag =
	    new artdaq::Fragment(ev_counter(), _fragment_ids[0], FragmentType::TRK, tstamp);
...
bool mu2e::TrackerBR::getNext_(artdaq::FragmentPtrs& Frags)
{
	...
	TLOG(TLVL_DBG) << "event: " << ev_counter() << "STARTING";
	//-----------------------------------------------------------------------------
	...
	if((CommandableFragmentGenerator::ev_counter() % _maxEventsPerSubrun) == 0)
	{
		...
		//-----------------------------------------------------------------------------
		// not sure I understand the logic of assigning the first event number in the
		// subrun here
		//-----------------------------------------------------------------------------
