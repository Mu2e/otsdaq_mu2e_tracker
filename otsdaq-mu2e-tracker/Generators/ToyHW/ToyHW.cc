...
ToyHW::ToyHW(fhicl::ParameterSet const& ps)
    ...
    , change_after_N_seconds_(
          ps.get<size_t>("change_after_N_seconds", std::numeric_limits<size_t>::max()))
    , pause_after_N_seconds_(ps.get<size_t>("pause_after_N_seconds", 0))
    , nADCChannels_after_N_seconds_(
          ps.get<size_t>("nADCChannels_after_N_seconds", nADCChannels_))
    ...
    , send_calls_(0)
{
	if(nADCChannels_ > maxADCcounts_ || nADCChannels_after_N_seconds_ > maxADCcounts_)
	{
		throw cet::exception("HardwareInterface")  // NOLINT(cert-err60-cpp)
		    ...
		    << " is larger than the \"maxADCcounts\" setting (currently at "
		    << maxADCcounts_ << ")";
	}

	bool planned_disruption = nADCChannels_after_N_seconds_ != nADCChannels_ ||
	                          exception_after_N_seconds_ || exit_after_N_seconds_ ||
	   ...
	   change_after_N_seconds_ == std::numeric_limits<size_t>::max())
	{
		throw cet::exception("HardwareInterface")
		    << "A FHiCL parameter designed to create a disruption has been "  // NOLINT(cert-err60-cpp)
...
void ToyHW::FillBuffer(char* buffer, size_t* bytes_read)
{
	...
	if(taking_data_)
	{
		TLOG(TLVL_DEBUG + 3) << "FillBuffer: Sleeping for " << throttle_usecs_
		                     << " microseconds";
		usleep(throttle_usecs_);

		auto elapsed_secs_since_datataking_start =
		    artdaq::TimeUtils::GetElapsedTime(start_time_);
		if(elapsed_secs_since_datataking_start < 0)
			elapsed_secs_since_datataking_start = 0;
		// nadccounts are the channels
		if(static_cast<size_t>(elapsed_secs_since_datataking_start) <
		       change_after_N_seconds_ ||
		   send_calls_ == 0)
		{
			TLOG(TLVL_DEBUG + 3)
			...
			*bytes_read = sizeof(demo::ToyFragment::Header) +
			              nADCChannels_ * sizeof(data_t);  // data size
		}
		else
		{
			...
			{
				TLOG(TLVL_ERROR) << "Pretending that the hardware has hung! Variable "
				                    "name for gdb: hardwareIsHung";
			...
			else
			{
				if((pause_after_N_seconds_ != 0u) &&
				   (static_cast<size_t>(elapsed_secs_since_datataking_start) %
				        change_after_N_seconds_ ==
				    0))
				{
					TLOG(TLVL_DEBUG + 3)
					    << "pausing " << pause_after_N_seconds_ << " seconds";
					sleep(pause_after_N_seconds_);
					TLOG(TLVL_DEBUG + 3) << "resuming after pause of "
					                     << pause_after_N_seconds_ << " seconds";
				}
				TLOG(TLVL_DEBUG + 3)
				    << "FillBuffer: Setting bytes_read to "
				    << sizeof(demo::ToyFragment::Header) +
				           nADCChannels_after_N_seconds_ * sizeof(data_t);
				*bytes_read = sizeof(demo::ToyFragment::Header) +
				              nADCChannels_after_N_seconds_ * sizeof(data_t);
			}
		}
		...
		auto* header = reinterpret_cast<demo::ToyFragment::Header*>(
		    buffer);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)

		header->event_size     = *bytes_read / sizeof(demo::ToyFragment::Header::data_t);
		header->trigger_number = 99;

		TLOG(TLVL_DEBUG + 3) << "FillBuffer: Generating nADCChannels ADC values ranging "
		                        "from 0 to max based on the desired distribution";
	...
	{
		throw cet::exception("ToyHW") << "Attempt to call FillBuffer when not sending "
		                                 "data";  // NOLINT(cert-err60-cpp)
	}
	...
	if(usecs_between_sends_ != 0)
	{
		if(send_calls_ > 0)
		{
			auto usecs_since_start =
			    artdaq::TimeUtils::GetElapsedTimeMicroseconds(start_time_);
			double delta = static_cast<double>(usecs_between_sends_ * send_calls_) -
			               usecs_since_start;
			TLOG(TLVL_DEBUG + 3)
			    << "FillBuffer send_calls=" << send_calls_
			    << " usecs_since_start=" << usecs_since_start << " delta=" << delta;
			if(delta > 0)
			{
				TLOG(TLVL_DEBUG + 3)
				    << "FillBuffer: Sleeping for " << delta << " microseconds";
				usleep(delta);
			}
		}
	}
...
void ToyHW::AllocateReadoutBuffer(char** buffer)
{
	*buffer =
	    reinterpret_cast<char*>(  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
	        new uint8_t[sizeof(demo::ToyFragment::Header) +
...
int ToyHW::NumADCBits() const
{
	...
	case demo::FragmentType::TOY2:
		...
		break;
	default:
		throw cet::exception("ToyHW")
		    << "Unknown board type " << fragment_type_ << " ("  // NOLINT(cert-err60-cpp)
...
{
	// Serial number is generated from the uniform distribution on initialization of the
	// class
