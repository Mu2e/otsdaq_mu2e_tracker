...
int Ui_PanelID_RW        (int Link, int Rw, int& PanelID, int PrintLevel = 0);
int Ui_ReadPanelID       (int Link, int PrintLevel = 0);
int Ui_WritePanelID      (int Link, int PanelID, int PrintLevel = 0);
std::string Ui_ReadSerialNumber  (const DTCLib::DTC_Link_ID& Link);
int Ui_InitRocReadoutMode(std::ostream* Stream = nullptr)      ;
int Ui_RebootMcu          (int Link);
int Ui_ResetLink         (int Link) ;
int Ui_RocConfigurePatternMode();
int Ui_RocSetDataVersion      (int Version, int LinkMask=0);
std::vector<DTCLib::roc_data_t> Ui_ReadDeviceID(DTCLib::DTC_Link_ID Link,
                                                 int                 PrintLevel = 0,
                                                 std::ostream&       Stream     = std::cout);
int Ui_FindAlignment (DTCLib::DTC_Link_ID Link, Alignment& Res);
int Ui_FindAlignments(int Link, int& NBitSlips, int PrintLevel, std::ostream& Stream = std::cout);
...
int Ui_MonicaVarLinkConfig   (std::ostream* Stream = nullptr);
