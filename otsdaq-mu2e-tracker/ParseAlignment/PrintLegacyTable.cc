...
void print_legacy_table(const Alignment& alignment)
{
	...
	std::cout << "('EyeMonitorWidth', " << alignment.EyeMonitorWidth() << ")"
	          << std::endl;
	std::cout << "('IfPatternCheck', " << alignment.IfPatternCheck() << ")" << std::endl;
	std::cout << "('FaultedADC', " << alignment.FaultedADC() << ")" << std::endl;

	std::cout.setf(std::ios_base::left);
	for(const auto& iteration : alignment.Iterations())
	{
		std::cout << "******   Iteration " << iteration.Index() << "   ******"
		          << std::endl;

		print_legacy_leading_row("ADCPhase",
		                         iteration.ADCPhase(),
		                         "",
		                         "Alignment",
		                         "Bitslip",
