...
// now reset the station (or cycle the LV on it), w/o the reset, can't proceed with the other side
// as the microcontroller gets stuck in some way
dtc_i->fDtc->SoftReset()
t.spi_program_digis(dtc_i,0,"HVV6.dat")
// reset the station (tower, test stand) again (or cycle the LV)
dtc_i->fDtc->SoftReset()
...
#+begin_src
auto dtc_i = dtc_init("mu2e-trk-08",0)
int link = 1;
int mnid = 52;
dtc_i->WritePanelID(link,mnid);
