...
void test1_read_data(int Link, int NEvents)
{
	...
	rc = dev->read_register(0x9138, 100, &res);
	printf("0x9138: SERDES Reset Done: 0x%08x\n", res);  // expect:
	...
	cfo.SendRequestsForRange(NEvents,
	                         DTCLib::DTC_EventWindowTag(uint64_t(0)),
	                         incrementTimestamp,
	                         cfodelay,
	                         requestsAhead,
	                         heartbeatsAfter);
	...
	{
		// cfo.SendRequestForTimestamp(DTCLib::DTC_EventWindowTag(uint64_t(i+1)),
		// heartbeatsAfter); std::this_thread::sleep_for(std::chrono::milliseconds(100));
		...
		rc = dev->read_register(0x9100, 100, &res);
		printf("DTC status       : 0x%08x\n", res);  // expect: 0x40808404
		rc = dev->read_register(0x91c8, 100, &res);
		printf("debug packet type: 0x%08x\n", res);  // expect: 0x00000000
	}
                     ...
                     const char* OutputFn          = nullptr)
{
	...
	for(int i = 0; i < NEvents; i++)
	{
		...
		{
			monica_digi_clear(dtc, Link);
			// monica_var_link_config(dtc,Link);                 // shouldn't need any
			// more
			dev->write_register(0x91a8, 100, heartbeatInterval);
		}
		//-----------------------------------------------------------------------------
		...
		//-----------------------------------------------------------------------------
		// 3. read SPI
		// after writing to reg 258, sleep foror 2 msec, then wait till reg 128 becomes
		// non-zero don't go below 1.5msec of sleep
		//-----------------------------------------------------------------------------
		printf(" -------------------- 3. read SPI:\n");
		...
		// int nw = dtc->ReadROCRegister(roc,129,100); printf("reg_0x%02x
		// val:0x%04x\n",129,nw);
