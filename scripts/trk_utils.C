...
//   dtc->WriteROCRegister(link, 8,0x030f,false,1000);             // configure ROC to
//   read all 4 lanes
...
//   dtc->WriteROCRegister(link,30,0x0000,false,1000);        // Set to zero, ie
//   STATUS=0x55 std::this_thread::sleep_for(std::chrono::microseconds(gSleepTimeROC));

//   // echo "Setting packet format version to 1"
//   // rocUtil -a 29  -w 1  -l $LINK write_register > /dev/null

//   dtc->WriteROCRegister(link,29,0x0001,false,1000);        // configure ROC to read all
//   4 lanes std::this_thread::sleep_for(std::chrono::microseconds(gSleepTimeROC));
// }
...
void monica_var_link_config(int LinkMask = 0, int PcieAddr = -1)
{
	...
	for(int i = 0; i < 6; i++)
	{
		...
		// dtc->WriteROCRegister(link,30,0x0000,false,1000);        // configure ROC to
		// read all 4 lanes
		// std::this_thread::sleep_for(std::chrono::microseconds(gSleepTimeROC));
		//-----------------------------------------------------------------------------
		// echo "Setting packet format version to 1"
...
// //-----------------------------------------------------------------------------
// mu2e_databuff_t* readDTCBuffer(mu2edev* device, bool& readSuccess, bool& timeout,
// size_t& sts) {
//   mu2e_databuff_t* buffer;
//   auto tmo_ms = 1500;
//   readSuccess = false;

//   sts = device->read_data(DTC_DMA_Engine_DAQ, reinterpret_cast<void**>(&buffer),
//   tmo_ms); std::this_thread::sleep_for(std::chrono::microseconds(gSleepTimeDTC));
...
//       readPtr = static_cast<uint8_t*>(readPtr) + sizeof(DTCLib::DTC_EventHeader) +
//       sizeof(DTCLib::DTC_SubEventHeader); std::vector<size_t> wordsToCheck{ 1, 2, 3, 7,
//       8 }; for (auto& word : wordsToCheck) 	{
