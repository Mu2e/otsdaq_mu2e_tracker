...
void dtc_print_roc_status(int Link, int PcieAddress = -1)
{
	...
	dat = dtc->ReadROCRegister(link, reg, 100);
	cout << Form(
	    "reg(%2i)             :     0x%04x : Fiber loss/lock counter\n", reg, dat);
	...
	iw  = (iw2 << 16) | iw1;
	cout << Form(
	    "reg(%i)<<16+reg(%i) : 0x%08x : SIZE_FIFO_FULL "
	    "[28]+STORE_POS[25:24]+STORE_CNT[19:0]\n",
	    reg + 1,
	    reg,
	    iw);
	...
	iw  = (iw2 << 16) | iw1;
	cout << Form(
	    "reg(%i)<<16+reg(%i) : 0x%08x : "
	    "SIZE_FIFO_EMPTY[28]+FETCH_POS[25:24]+FETCH_CNT[19:0]\n",
	    reg + 1,
	    reg,
	    iw);
	...
	iw  = (iw2 << 16) | iw1;
	cout << Form(
	    "reg(%i)<<16+reg(%i) : 0x%08x : Num PREFETCH seen\n\n", reg + 1, reg, iw);
	...
	iw  = (iw2 << 16) | iw1;
	cout << Form(
	    "reg(%i)<<16+reg(%i) : 0x%08x : Num DATA REQ read from DDR\n", reg + 1, reg, iw);
	...
	iw  = (iw2 << 16) | iw1;
	cout << Form(
	    "reg(%i)<<16+reg(%i) : 0x%08x : Num DATA REQ sent to DTC\n", reg + 1, reg, iw);
	...
	iw  = (iw2 << 16) | iw1;
	cout << Form(
	    "reg(%i)<<16+reg(%i) : 0x%08x : Num DATA REQ with null data\n", reg + 1, reg, iw);
	...
	dat = dtc->ReadROCRegister(link, reg, 100);
	cout << Form(
	    "reg(%2i)             :     0x%04x : Num HB tag inconsistencies\n", reg, dat);
	...
	dat = dtc->ReadROCRegister(link, reg, 100);
	cout << Form(
	    "reg(%2i)             :     0x%04x : Num DREQ tag inconsistencies\n", reg, dat);
