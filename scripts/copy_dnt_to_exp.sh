...
#
# call format: copy_dnt_to_exp.sh run1 run2 [doit] 
#
...
for rn in `seq $rn1 $rn2` ; do
    irn=`printf "%06i" $rn`
    for f in `ls /data/tracker/vst/$name_stub/digi_ntuples/make_digi_ntuple_* | grep $rn` ; do
        bn=`basename $f`
        dsconf=`echo $bn | awk -F . '{print $4}'`
        cmd="scp $f murat@mu2egpvm06:/exp/mu2e/data/projects/tracker/vst/digi_ntuples/."
        echo "$cmd"
        if [ ".$doit" != "." ] ; then 
            # echo doit=$doit
            $cmd ; echo rc=$? ; 
