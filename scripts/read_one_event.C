...
void analyze(uint16_t* Ptr, int NWords)
{
	...
	printf(
	    " adc[ 0] adc[ 1] adc[ 2] adc[ 3] adc[ 4] adc[ 5] adc[ 6] adc[ 7] adc[ 8] adc[ "
	    "9] adc[10]");
	...
	for(int i = 0; i < nhits; i++)
	{
		...
		adc[1] =
		    reverseBits((uint32_t)((dat[6] & 0xfc00) >> 10) + ((dat[7] & 0x000f) << 6));
		...
		printf("%4i %5i %10i %10i %4i %4i %4i %4i",
		       i,
		       sid,
		       tdc0,
		       tdc1,
		...
		printf(
		    " %7i %7i %7i %7i %7i %7i", adc[0], adc[1], adc[2], adc[3], adc[4], adc[5]);
...
void read_one_event(int NEvents = 1, int Debug = 0)
{
	...
	//-----------------------------------------------------------------------------
	// for some reason, it seems critical that we reset device time after sending requests
	// for range.
	// ... double check...
	...
	int i = 0;
	cfo.SendRequestsForRange(NEvents + 1,
	                         DTC_EventWindowTag(uint64_t(i)),
	                         incrementTimestamp,
	                         heartbeatInterval,
	                         requestsAhead,
	                         heartbeatsAfter);
	...
	// print_roc_registers(&dtc,DTCLib::DTC_Link_0,"001 [after
	// cfo.SendRequestForTimestamp]");
