	...
	for(int i = 0; i < 6; i++)
	{
		...
		if(used != 0)
		{
			...
			std::this_thread::sleep_for(
			    std::chrono::microseconds(10 * trk_daq::gSleepTimeROCReset));

			dtc->WriteROCRegister(
			    link, 8, 0x0010, false, 1000);  // configure ROC to send patterns
			// std::this_thread::sleep_for(std::chrono::microseconds(10*trk_daq::gSleepTimeROC));

			dtc->WriteROCRegister(
			    link, 30, 0x0000, false, 1000);  // r30: mode, write 0 into it
			// std::this_thread::sleep_for(std::chrono::microseconds(10*trk_daq::gSleepTimeROC));

			dtc->WriteROCRegister(
			    link, 29, 0x0001, false, 1000);  // r29: data version, currently 1
			// std::this_thread::sleep_for(std::chrono::microseconds(10*trk_daq::gSleepTimeROC));
		}
	}
