...
//-----------------------------------------------------------------------------
void test_buffer_sync(int NEvents) {
...
//-----------------------------------------------------------------------------
  ...
  rc = dev->read_register(0x9138,100,&res); printf("SERDES Reset Done: 0x%08x\n",res); // expect: 
//-----------------------------------------------------------------------------
// var_pattern_config
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  ...
  for (int i=0; i<NEvents+1; i++) {
    ...
    dtc.WriteROCRegister(DTCLib::DTC_Link_0,14,     1,false,100); // reset ROC
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    dtc.WriteROCRegister(DTCLib::DTC_Link_0, 8,0x0010,false,100); // configure ROC to send patterns
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    dtc.WriteROCRegister(DTCLib::DTC_Link_0,30,     0,false,100); // configure STATUS_BIT in MODE=0
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
